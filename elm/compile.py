...
class TempDir:
    ...
    def __enter__(self):
        os.makedirs(TempDir.PACKAGES_DIR)
    ...
    def __exit__(self, exception_type, exception_value, traceback):
        ...
        # Follow example in https://docs.python.org/3/library/os.html#os.walk
        for root, dirs, files in os.walk('elm-home', topdown=False, followlinks=False):
            for name in files:
                os.remove(os.path.join(root, name))
            for name in dirs:
                target_link = os.path.join(root, name)
...
with TempDir():
    ...
    for package_dir in sys.argv[7:]:
        ...
        internal_package_dir = os.path.join(TempDir.PACKAGES_DIR, metadata["name"])
    ...
    with open(os.path.join(TempDir.PACKAGES_DIR, "versions.dat"), "wb") as f:
